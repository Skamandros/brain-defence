    ...
    def __init__(self):
        ...
        for level, variants_in_level in enumerate(intro_variants):
            ...
            for i in range(0, variants_in_level):
                intro_music.append(arcade.load_sound(
                    RESOURCE_DIR.joinpath(
                        "sound/Level{0:02d}_Intro{1:02d}.mp3".format(level + 1, i + 1)),
                    True))
        ...
        for level, events_in_level in enumerate(events_per_level):
            ...
            for i in range(0, events_in_level):
                events.append(arcade.load_sound(
                    RESOURCE_DIR.joinpath(
                        "sound/Level{0:02d}_Event{1:02d}.mp3".format(level + 1, i + 1)),
                    True))
