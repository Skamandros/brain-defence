...
class Icon(arcade.sprite.Sprite):
    def __init__(self, imagefilepath, **kwargs):
        super().__init__(imagefilepath, **kwargs)
        self.scale = kwargs.get("scale", 0.1)


class RotatingIcon(Icon):
    def __init__(self, imagefilepath, **kwargs):
        max_angle = kwargs.pop("max_angle", None)
        change_angle = kwargs.pop("change_angle", 1)
        super().__init__(imagefilepath, **kwargs)
    ...
    def update(self):
        ...
        if self.max_angle is not None:
            if (abs(self.angle) % 360) > self.max_angle:
                self.change_angle *= -1
...
class HUD:
    """Collection of widgets, Icons, counters..."""
